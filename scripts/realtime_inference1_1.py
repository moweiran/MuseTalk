...
class Avatar:
    ...
    def process_frames(self, res_frame_queue, video_len, skip_save_images):
        ...
        while True:
            ...
            # Use modulo indexing to avoid deep copying entire frames
            idx_mod = self.idx % len(self.coord_list_cycle)
            bbox = self.coord_list_cycle[idx_mod]
            # Only use the frame reference instead of deep copying
            ori_frame = self.frame_list_cycle[idx_mod]
            x1, y1, x2, y2 = bbox
            
            try:
                # Resize with explicit interpolation for better performance
                res_frame = cv2.resize(res_frame.astype(np.uint8), (x2 - x1, y2 - y1), interpolation=cv2.INTER_LINEAR)
            except:
                continue
                
            # Use modulo indexing for masks as well
            mask_idx = self.idx % len(self.mask_list_cycle)
            mask = self.mask_list_cycle[mask_idx]
            mask_crop_box = self.mask_coords_list_cycle[mask_idx]
            
            # Perform blending
            combine_frame = get_image_blending(ori_frame, res_frame, bbox, mask, mask_crop_box)
            
            # Asynchronously write images using thread pool
            if not skip_save_images and executor:
                output_path = f"{self.avatar_path}/tmp/{str(self.idx).zfill(8)}.png"
                # Submit write operation to thread pool
                future = executor.submit(cv2.imwrite, output_path, combine_frame)
                write_futures.append(future)
                print(f"Queued image save: {output_path}")
                
            self.idx = self.idx + 1
            
        # Wait for all image writes to complete
        if not skip_save_images and executor:
