...
#         self.stop_event = threading.Event()  # 用于控制线程停止
#         self.current_rtmp_url = None
#         self.completion_event = threading.Event()  # Add this
    
#     def start_streaming(self, audio_path:str, avatar_path:str, rtmp_url:str):
#         """
#         开始播放MP4文件到RTMP服务器
#         """
#         self.completion_event.clear()  # Clear completion event
#         stream_cmd = [
#             'ffmpeg',
#             '-re',
#             '-r', '30',
#             '-f', 'image2',
#             '-i', f"{avatar_path}/tmp/%08d.png",
#             '-i', audio_path,
#             '-c:v', 'libx264',  # 指定视频编码器
#             '-c:a', 'aac',
#             '-preset', 'medium',
...
#     def play(self, audio_path:str, avatar_path:str, rtmp_url):
#         # 检查是否已经有线程在运行
#         if self.mp4_thread and self.mp4_thread.is_alive():
#             if self.current_rtmp_url == rtmp_url:
#                 print("Streaming is already running with the same URL. No new thread will be started.")
...
#         self.stop_event.clear()  # 确保停止事件未设置
#         self.completion_event.clear()  # Clear completion event
#         self.mp4_thread = threading.Thread(target=self.start_streaming, args=(audio_path, avatar_path, rtmp_url))
#         self.mp4_thread.start()
#         print("Streaming started.")
